...
const SwipeCard = React.forwardRef(({ data, industry, index, isFront, onSwipe, direction }, ref) => {
  ...
  return (
    <motion.div
      ...
      // exit={{ x: direction === 'left' ? -800 : direction === 'right' ? 800 : (x.get() < 0 ? -800 : 800), opacity: 0, rotate: direction === 'left' ? -15 : direction === 'right' ? 15 : (x.get() < 0 ? -15 : 15), transition: { type: "spring", stiffness: 200, damping: 10, duration: 6 } }}
